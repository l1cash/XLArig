...
void xlarig::FindSharesKernel::enqueue(cl_command_queue queue, size_t threads)
{
...
void xlarig::FindSharesKernel::setArgs(cl_mem hashes, cl_mem shares)
{
...
void xlarig::FindSharesKernel::setTarget(uint64_t target)
{
    setArg(1, sizeof(uint64_t), &target);
}


void xlarig::FindSharesKernel::setNonce(uint32_t nonce)
{
