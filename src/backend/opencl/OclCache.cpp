...
cl_program xlarig::OclCache::build(const IOclRunner *runner)
{
...
#       ifdef _WIN32
        fileName = prefix() + "\\xlarig\\.cache\\" + cacheKey(runner) + ".bin";
#       else
...
std::string xlarig::OclCache::cacheKey(const char *deviceKey, const char *options, const char *source)
{
...
std::string xlarig::OclCache::cacheKey(const IOclRunner *runner)
{
    return cacheKey(runner->deviceKey(), runner->buildOptions(), runner->source());
}


void xlarig::OclCache::save(cl_program program, const std::string &fileName)
{
    size_t size = 0;
