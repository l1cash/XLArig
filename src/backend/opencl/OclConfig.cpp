...
xlarig::OclConfig::OclConfig() :
    m_platformVendor(kAMD)
{
}


xlarig::OclPlatform xlarig::OclConfig::platform() const
{
...
rapidjson::Value xlarig::OclConfig::toJSON(rapidjson::Document &doc) const
{
...
std::vector<xlarig::OclLaunchData> xlarig::OclConfig::get(const Miner *miner, const Algorithm &algorithm, const OclPlatform &platform, const std::vector<OclDevice> &devices, const char *tag) const
{
...
void xlarig::OclConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void xlarig::OclConfig::generate()
{
    ...
    count += xlarig::generate(kCn, m_threads, Algorithm::CN_0, devices);
    count += xlarig::generate(kCn2, m_threads, Algorithm::CN_2, devices);
...
#   ifdef XMRIG_ALGO_CN_GPU
    count += xlarig::generate(kCnGPU, m_threads, Algorithm::CN_GPU, devices);
#   endif

#   ifdef XMRIG_ALGO_CN_LITE
    count += xlarig::generate(kCnLite, m_threads, Algorithm::CN_LITE_1, devices);
...
#   ifdef XMRIG_ALGO_CN_HEAVY
    count += xlarig::generate(kCnHeavy, m_threads, Algorithm::CN_HEAVY_0, devices);
#   endif

#   ifdef XMRIG_ALGO_CN_PICO
    count += xlarig::generate(kCnPico, m_threads, Algorithm::CN_PICO_0, devices);
#   endif

#   ifdef XMRIG_ALGO_RANDOMX
    count += xlarig::generate(kRx, m_threads, Algorithm::RX_0, devices);
    count += xlarig::generate(kRxWOW, m_threads, Algorithm::RX_WOW, devices);
#   endif
...
void xlarig::OclConfig::setDevicesHint(const char *devicesHint)
{
...
void xlarig::OclConfig::setPlatform(const rapidjson::Value &platform)
{
