...
xlarig::OclDevice::OclDevice(uint32_t index, cl_device_id id, cl_platform_id platform) :
    m_id(id),
    m_platform(platform),
...
size_t xlarig::OclDevice::freeMemSize() const
{
    return std::min(maxMemAllocSize(), globalMemSize());
}


size_t xlarig::OclDevice::globalMemSize() const
{
    return OclLib::getUlong(id(), CL_DEVICE_GLOBAL_MEM_SIZE);
}


size_t xlarig::OclDevice::maxMemAllocSize() const
{
    return OclLib::getUlong(id(), CL_DEVICE_MAX_MEM_ALLOC_SIZE);
}


xlarig::String xlarig::OclDevice::printableName() const
{
...
uint32_t xlarig::OclDevice::clock() const
{
    return OclLib::getUint(id(), CL_DEVICE_MAX_CLOCK_FREQUENCY);
}


void xlarig::OclDevice::generate(const Algorithm &algorithm, OclThreads &threads) const
{
...
void xlarig::OclDevice::toJSON(rapidjson::Value &out, rapidjson::Document &doc) const
{
