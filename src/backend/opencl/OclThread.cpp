...
xlarig::OclThread::OclThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
bool xlarig::OclThread::isEqual(const OclThread &other) const
{
...
rapidjson::Value xlarig::OclThread::toJSON(rapidjson::Document &doc) const
{
