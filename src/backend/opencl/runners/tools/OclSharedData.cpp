...
cl_mem xlarig::OclSharedData::createBuffer(cl_context context, size_t size, size_t &offset)
{
...
uint64_t xlarig::OclSharedData::adjustDelay(size_t id)
{
    if (m_threads < 2) {
        return 0;
    }
...
uint64_t xlarig::OclSharedData::resumeDelay(size_t id)
{
    if (m_threads < 2) {
        return 0;
    }
...
void xlarig::OclSharedData::release()
{
...
void xlarig::OclSharedData::setResumeCounter(uint32_t value)
{
    if (m_threads < 2) {
        return;
    }
...
void xlarig::OclSharedData::setRunTime(uint64_t time)
{
...
#ifdef XMRIG_ALGO_RANDOMX
cl_mem xlarig::OclSharedData::dataset() const
{
    if (!m_dataset) {
...
void xlarig::OclSharedData::createDataset(cl_context ctx, const Job &job, bool host)
{
    if (m_dataset) {
        return;
    }
