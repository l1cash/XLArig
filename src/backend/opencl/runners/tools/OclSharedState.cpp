...
xlarig::OclSharedData &xlarig::OclSharedState::get(uint32_t index)
{
    return map[index];
}


void xlarig::OclSharedState::release()
{
    for (auto &kv : map) {
        kv.second.release();
    }
...
void xlarig::OclSharedState::start(const std::vector<OclLaunchData> &threads, const Job &job)
{
    assert(map.empty());
