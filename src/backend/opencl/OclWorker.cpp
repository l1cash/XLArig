...
xlarig::OclWorker::OclWorker(size_t id, const OclLaunchData &data) :
...
xlarig::OclWorker::~OclWorker()
{
    delete m_runner;
}


bool xlarig::OclWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t xlarig::OclWorker::intensity() const
{
    return m_runner ? m_runner->intensity() : 0;
}


void xlarig::OclWorker::start()
{
...
bool xlarig::OclWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::OPENCL) == 0) {
        return false;
    }
...
void xlarig::OclWorker::storeStats(uint64_t t)
{
    if (!isReady()) {
        return;
    }
