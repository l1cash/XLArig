...
const char *xlarig::ocl_tag()
{
    return tag;
}


xlarig::OclBackend::OclBackend(Controller *controller) :
...
bool xlarig::OclBackend::isEnabled() const
{
    return d_ptr->controller->config()->cl().isEnabled() && OclLib::isInitialized() && d_ptr->platform.isValid() && !d_ptr->devices.empty();
}


bool xlarig::OclBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cl().threads().get(algorithm).isEmpty();
}


const xlarig::Hashrate *xlarig::OclBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const xlarig::String &xlarig::OclBackend::profileName() const
{
    return d_ptr->profileName;
}


const xlarig::String &xlarig::OclBackend::type() const
{
    return kType;
}


void xlarig::OclBackend::prepare(const Job &)
{
}


void xlarig::OclBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void xlarig::OclBackend::setJob(const Job &job)
{
...
void xlarig::OclBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void xlarig::OclBackend::tick(uint64_t ticks)
{
...
#ifdef XMRIG_FEATURE_API
rapidjson::Value xlarig::OclBackend::toJSON(rapidjson::Document &doc) const
{
...
void xlarig::OclBackend::handleRequest(IApiRequest &)
{
}
#endif
