...
template<>
inline uint32_t *xlarig::WorkerJob<1>::nonce(size_t)
{
...
template<>
inline void xlarig::WorkerJob<1>::nextRound(uint32_t rounds, uint32_t roundSize)
{
    m_rounds[index()]++;
...
template<>
inline void xlarig::WorkerJob<1>::save(const Job &job, uint32_t reserveCount)
{
    m_index           = job.index();
    m_jobs[index()]   = job;
    m_rounds[index()] = 0;
