...
bool xlarig::Rx::init(const Job &job, int initThreads, bool hugePages, bool numa, IRxListener *listener)
{
...
bool xlarig::Rx::isReady(const Job &job)
{
    std::lock_guard<std::mutex> lock(mutex);
...
xlarig::RxDataset *xlarig::Rx::dataset(const Job &job, uint32_t nodeId)
{
    std::lock_guard<std::mutex> lock(mutex);
    if (!d_ptr->isReady(job)) {
        return nullptr;
...
std::pair<unsigned, unsigned> xlarig::Rx::hugePages()
{
