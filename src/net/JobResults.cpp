...
void xlarig::JobResults::setListener(IJobResultListener *listener, bool hwAES)
{
...
void xlarig::JobResults::submit(const Job &job, uint32_t nonce, const uint8_t *result)
{
    submit(JobResult(job, nonce, result));
}


void xlarig::JobResults::submit(const JobResult &result)
{
    assert(handler != nullptr);
...
#if defined(XMRIG_FEATURE_OPENCL) || defined(XMRIG_FEATURE_CUDA)
void xlarig::JobResults::submit(const Job &job, uint32_t *results, size_t count)
{
    if (handler) {
